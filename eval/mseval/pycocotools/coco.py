...
class COCO:
    def __init__(self, annotation_file=None, imgidStr = 'image_id', imgOrVid = 'images'):
        """
    ...
    def createIndex(self):
        ...
        imgToAnns = {ann[self.imgidStr]: [] for ann in self.dataset['annotations']}
        anns =      {ann['id']:       [] for ann in self.dataset['annotations']}
        for ann in self.dataset['annotations']:
            imgToAnns[ann[self.imgidStr]] += [ann]
            anns[ann['id']] = ann

        imgs      = {im['id']: {} for im in self.dataset[self.imgOrVid]}
        for img in self.dataset[self.imgOrVid]:
            imgs[img['id']] = img
        ...
        if self.dataset['type'] == 'instances':
            ...
            for ann in self.dataset['annotations']:
                catToImgs[ann['category_id']] += [ann[self.imgidStr]]
    ...
    def loadRes(self, resFile):
        ...
        """
        res = COCO(None, imgidStr = self.imgidStr, imgOrVid = self.imgOrVid)
        res.dataset[self.imgOrVid] = [img for img in self.dataset[self.imgOrVid]]
        ...
        annsImgIds = [ann[self.imgidStr] for ann in anns]
        assert set(annsImgIds) == (set(annsImgIds) & set(self.getImgIds())), \
               'Results do not correspond to current coco set'
        if 'caption' in anns[0]:
            imgIds = set([img['id'] for img in res.dataset[self.imgOrVid]]) & set([ann[self.imgidStr] for ann in anns])
            res.dataset[self.imgOrVid] = [img for img in res.dataset[self.imgOrVid] if img['id'] in imgIds]
